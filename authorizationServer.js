...
app.get("/authorize", function(req, res){
  ...
  console.log(`I am authorize endpoint ${req.query.client_id}`)
...
app.post("/token", function(req, res){
  ...
  // log: to check we have reached token endpoint
  console.log('I am token endpoint');

  //callback from client header information 
  // { 
  // "Content-Type":"application/x-www-form-urlencoded",
  // "Authorization":"Basicb2F1dGgtY2xpZW50LTE6b2F1dGgtY2xpZW50LXNlY3JldC0x"
  // }
  
  // Checking header for token endpoint
  console.log(`I am headers value from token endpoint ${JSON.stringify(req.headers)}`)
  ...
  // Check if auth exist at all
  console.log(`Check if auth exists at all  ${auth}`)
	
  // Basicb2F1dGgtY2xpZW50LTE6b2F1dGgtY2xpZW50LXNlY3JldC0x
  if (auth) {
		// check the auth header
		var clientCredentials = Buffer.from(auth.slice('Basic '.length), 'base64').toString().split(':');

    console.log(`Checking cleintCredentials ${clientCredentials}`)
